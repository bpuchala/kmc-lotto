...
class EventRateNodeDataTest : public testing::Test {
 protected:
  ...
  bool has_event_id(const lotto::EventRateNodeData<ID>& data) const {
    return data.event_id.has_value();
  }
};
...
class EventRateTreeTest : public testing::Test {
 protected:
  ...
  const std::map<ID, lotto::Index>& event_to_leaf_index() const {
    return tree_ptr->event_to_leaf_index;
  }
...
TEST_F(EventRateTreeTest, UpdateRate) {
  // Checks that the individual and total rates change appropriately upon
  // updating
  int n_updates = 100;
...
TEST_F(EventRateTreeTest, RandomQuery) {
  // Check thats querying the tree returns the correct event ID, based on the
  // cumulative rates
...
TEST_F(EventRateTreeTest, EdgeQuery) {
  // Checks that correct event is selected in edge case where query value is
  // exactly equal to a cumulative rate
