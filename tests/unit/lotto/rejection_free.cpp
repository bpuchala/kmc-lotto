 ...
 protected:
  ...
  void SetUp() override {
    ...
    one_hot_calculator_ptr =
        std::make_shared<OneHotRateCalculator<ID>>(event_ids[0]);
    ...
    uniform_selector_ptr = std::make_unique<
        lotto::RejectionFreeEventSelector<ID, UniformRateCalculator<ID>>>(
        uniform_calculator_ptr, event_ids, complete_impact_table);
    uniform_no_impact_selector_ptr = std::make_unique<
        lotto::RejectionFreeEventSelector<ID, UniformRateCalculator<ID>>>(
        uniform_calculator_ptr, event_ids, empty_impact_table);
    even_odd_selector_ptr = std::make_unique<
        lotto::RejectionFreeEventSelector<ID, EvenOddRateCalculator>>(
  ...
  std::unique_ptr<lotto::RejectionFreeEventSelector<ID, EvenOddRateCalculator>>
      even_odd_selector_ptr;
};
...
TEST_F(RejectionFreeEventSelectorTest, AverageTimeStep) {
  // Checks if the average time step is correct when all events have the same
  // rate
  ...
  for (int i = 1; i <= n_rates; ++i) {
    ...
    check_samples_from_log_inverse_distribution(1.0 / (event_ids.size() * r0),
                                                time_step_samples);
  }
}
