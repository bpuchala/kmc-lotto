...
/// Returns the standard error of the mean given the standard deviation and the
/// number of samples
...
/// Checks if the sample mean deviates from its true value by more than
/// sigma_tolerance times the standard error
inline void check_deviation_of_mean(double sample_mean, double true_mean,
                                    double standard_error_of_mean,
                                    double sigma_tolerance) {
  EXPECT_LE(std::abs(sample_mean - true_mean),
            sigma_tolerance * standard_error_of_mean);
  return;
}
...
template <typename T>
void check_samples_from_uniform_distribution(T min_value, T max_value,
  ...
  double true_standard_deviation =
      (double)(max_value - min_value) / std::sqrt(12.0);
  double sample_mean = mean(samples);
  double standard_error =
      standard_error_of_mean(true_standard_deviation, samples.size());
  check_deviation_of_mean(sample_mean, true_mean, standard_error, TEST_SIGMA);
}

/// Checks that samples satisfy bounds and expected mean for an inverse log
/// distribution
//  a*ln(1/x), where x is uniformly distributed between 0 and 1,
//  which has mean and standard deviation a
inline void check_samples_from_log_inverse_distribution(
    double a, std::vector<double> samples) {
