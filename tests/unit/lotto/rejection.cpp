...
class RejectionEventSelectorTest : public testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    one_hot_calculator_ptr =
        std::make_shared<OneHotRateCalculator<ID>>(event_id_list[0]);
    double rate_upper_bound = 1.0;
    uniform_calculator_ptr =
        std::make_shared<UniformRateCalculator<ID>>(rate_upper_bound);

    // Set up event selectors
    one_hot_selector_ptr = std::make_unique<
        lotto::RejectionEventSelector<ID, OneHotRateCalculator<ID>>>(
        one_hot_calculator_ptr, rate_upper_bound, event_id_list);
    uniform_selector_ptr = std::make_unique<
        lotto::RejectionEventSelector<ID, UniformRateCalculator<ID>>>(
  ...
  // Event selectors, stored with pointers because they have no default
  // constructor
  std::unique_ptr<lotto::RejectionEventSelector<ID, OneHotRateCalculator<ID>>>
      one_hot_selector_ptr;
  std::unique_ptr<lotto::RejectionEventSelector<ID, UniformRateCalculator<ID>>>
      uniform_selector_ptr;
...
TEST_F(RejectionEventSelectorTest, AverageTimeStep) {
  // Checks if the average time step is correct when all events have rates equal
  // to the rate upper bound
  ...
  for (int i = 1; i <= n_rates; ++i) {
    ...
    check_samples_from_log_inverse_distribution(
        1.0 / (event_id_list.size() * r0), time_step_samples);
  }
}
