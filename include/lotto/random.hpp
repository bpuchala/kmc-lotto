...
public:
    /// Constructor, automatically construct and seed from random device if engine is empty
    RandomGeneratorT(std::shared_ptr<EngineType> engine = std::shared_ptr<EngineType>())
        : engine(engine),
    ...
    RealType sample_unit_interval() { return unit_interval_distribution(*engine); }
    ...
    std::shared_ptr<EngineType> get_engine() const {
      return engine;
    }
